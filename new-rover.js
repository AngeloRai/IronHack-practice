...
function commandRover(theRover, orders) {
    for (let i = 0; i < orders.length; i++) {
        ...
        switch (order) {
            ...
            case 'r' || 'R':
                turnRight(theRover)
                console.log(`"Right" command received and the current direction\
                is "${theRover.direction.toUpperCase()}"`);
                break;
            case 'f' || 'F':
            ...
            case 'b' || 'B':
                ...
                break;
            default:
                console.log("Guess that command didn't work! Command the rover with the letters\
                 'l', 'r', 'f' and 'b'.");
        }
    }
}
...
function moveForward(theRover) {
    ...
    return console.log("Look out! We are falling off our planet.");
